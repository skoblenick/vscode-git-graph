...
function renderGraphView(view: GitGraphView) {
	...
	config.graph.grid.expandY = expandedCommitElem !== null
		? expandedCommitElem.getBoundingClientRect().height
		: cdvHeight;
	config.graph.grid.y = commits.length > 0 && tableElem.children.length > 0
		? (tableElem.children[0].clientHeight - headerHeight - (expandedCommit !== null ? cdvHeight : 0)) / commits.length
		: config.graph.grid.y;
...
function renderTableView(view: GitGraphView) {
	...
	const currentHash = commits.length > 0 && commits[0].hash === UNCOMMITTED ? UNCOMMITTED : commitHead;
	const vertexColours = graph.getVertexColours();
	const widthsAtVertices = config.referenceLabels.branchLabelsAlignedToGraph ? graph.getWidthsAtVertices() : [];
	...
	for (let i = 0; i < commits.length; i++) {
		...
		const commitDot = commit.hash === commitHead
		...
		html += '<tr class="commit' + (commit.hash === currentHash ? ' current' : '') + (mutedCommits[i] ? ' mute' : '') + '"' + (commit.hash !== UNCOMMITTED ? '' : ' id="uncommittedChanges"') + ' data-id="' + i + '" data-color="' + vertexColours[i] + '">' +
			(config.referenceLabels.branchLabelsAlignedToGraph ? '<td>' + (refBranches !== '' ? '<span style="margin-left:' + (widthsAtVertices[i] - 4) + 'px"' + refBranches.substring(5) : '') + '</td><td><span class="description">' + commitDot : '<td></td><td><span class="description">' + commitDot + refBranches) + (config.referenceLabels.tagLabelsOnRight ? message + refTags : refTags + message) + '</span></td>' +
			(colVisibility.date ? '<td class="dateCol text" title="' + date.title + '">' + date.formatted + '</td>' : '') +
			(colVisibility.author ? '<td class="authorCol text" title="' + escapeHtml(commit.author + ' <' + commit.email + '>') + '">' + (config.fetchAvatars ? '<span class="avatar" data-email="' + escapeHtml(commit.email) + '">' + (typeof avatars[commit.email] === 'string' ? '<img class="avatarImg" src="' + avatars[commit.email] + '">' : '') + '</span>' : '') + escapeHtml(commit.author) + '</td>' : '') +
	...
	footerElem.innerHTML = moreCommitsAvailable ? '<div id="loadMoreCommitsBtn" class="roundedBtn">Load More Commits</div>' : '';
...
function renderUncommittedChanges(view: GitGraphView) {
	const commits = view.getCommits();
	const colVisibility = getColumnVisibility(view), date = formatShortDate(commits[0].date);
	document.getElementById('uncommittedChanges')!.innerHTML = '<td></td><td><b>' + escapeHtml(commits[0].message) + '</b></td>' +
		(colVisibility.date ? '<td class="dateCol text" title="' + date.title + '">' + date.formatted + '</td>' : '') +
...
function renderTagDetails(view: GitGraphView, tagName: string, commitHash: string, details: GG.GitTagDetails) {
	const commits = view.getCommits();
	const gitRepos = view.getGitRepos();
	const currentRepo = view.getCurrentRepo();
	const config = view.getConfig();
...
function renderRepoDropdownOptions(view: GitGraphView, repo?: string) {
	view.getRepoDropdown().setOptions(getRepoDropdownOptions(view.getGitRepos()), [repo || view.getCurrentRepo()]);
}
...
function makeTableResizable(view: GitGraphView) {
	...
	if (columnWidths[0] !== COLUMN_AUTO) {
		...
		let colWidth = cols[0].offsetWidth, graphWidth = graph.getContentWidth();
