...
function setCdvHeight(view: GitGraphView, elem: HTMLElement, isDocked: boolean) {
	const gitRepos = view.getGitRepos();
	const currentRepo = view.getCurrentRepo();
	let height = gitRepos[currentRepo].cdvHeight, windowHeight = window.innerHeight;
...
function cdvUpdateFileState(view: GitGraphView, file: GG.GitFileChange, fileElem: HTMLElement, isReviewed: boolean | null, fileWasViewed: boolean) {
	const expandedCommit = view.getExpandedCommit(), filesElem = document.getElementById('cdvFiles'), filePath = file.newFilePath;
...
function changeFileViewType(view: GitGraphView, type: GG.FileViewType) {
	const expandedCommit = view.getExpandedCommit(), filesElem = document.getElementById('cdvFiles');
...
function makeCdvFileViewInteractive(view: GitGraphView) {
	...
	const triggerViewFileDiff = (file: GG.GitFileChange, fileElem: HTMLElement) => {
		...
		const commitLookup = view.getCommitLookup();
		const currentRepo = view.getCurrentRepo();
		let commit = commits[commitLookup[expandedCommit.commitHash]], fromHash: string, toHash: string, fileStatus = file.type;
		if (expandedCommit.compareWithHash !== null) {
	...
	const triggerCopyFilePath = (file: GG.GitFileChange, absolute: boolean) => {
		sendMessage({ command: 'copyFilePath', repo: view.getCurrentRepo(), filePath: file.newFilePath, absolute: absolute });
	};
	...
	const triggerResetFileToRevision = (file: GG.GitFileChange, fileElem: HTMLElement) => {
		...
		dialog.showConfirmation('Are you sure you want to reset <b><i>' + escapeHtml(file.newFilePath) + '</i></b> to it\'s state at commit <b><i>' + abbrevCommit(commitHash) + '</i></b>? Any uncommitted changes made to this file will be overwritten.', 'Yes, reset file', () => {
			runAction({ command: 'resetFileToRevision', repo: view.getCurrentRepo(), commitHash: commitHash, filePath: file.newFilePath }, 'Resetting file');
		}, {
	...
	const triggerViewFileAtRevision = (file: GG.GitFileChange, fileElem: HTMLElement) => {
		...
		cdvUpdateFileState(view, file, fileElem, true, true);
		sendMessage({ command: 'viewFileAtRevision', repo: view.getCurrentRepo(), hash: getCommitHashForFile(file, expandedCommit), filePath: file.newFilePath });
	};
	...
	const triggerViewFileDiffWithWorkingFile = (file: GG.GitFileChange, fileElem: HTMLElement) => {
		...
		sendMessage({ command: 'viewDiffWithWorkingFile', repo: view.getCurrentRepo(), hash: getCommitHashForFile(file, expandedCommit), filePath: file.newFilePath });
	};
	...
	const triggerOpenFile = (file: GG.GitFileChange, fileElem: HTMLElement) => {
		...
		cdvUpdateFileState(view, file, fileElem, true, true);
		sendMessage({ command: 'openFile', repo: view.getCurrentRepo(), hash: getCommitHashForFile(file, expandedCommit), filePath: file.newFilePath });
	};
	...
	addListenerToClass('fileTreeFileRecord', 'contextmenu', (e: Event) => {
		...
		], false, target, <MouseEvent>e, view.isCdvDocked() ? document.body : view.getViewElem(), () => {
			expandedCommit.contextMenuOpen.fileView = -1;
		});
	});
}
...
function renderCdvExternalDiffBtn(view: GitGraphView) {
	...
	alterClass(externalDiffBtnElem, CLASS_ENABLED, gitConfig !== null && (gitConfig.diffTool !== null || gitConfig.guiDiffTool !== null));
	const toolName = gitConfig !== null
...
function renderCommitDetailsView(view: GitGraphView, refresh: boolean) {
	...
	const externalDiffPossible = !expandedCommit.loading && (expandedCommit.compareWithHash !== null || commits[commitLookup[expandedCommit.commitHash]].parents.length > 0);
	...
	if (!refresh) {
		if (isDocked) {
			...
			let elemTop = controlsElem.clientHeight + elem.offsetTop, cdvHeight = gitRepos[currentRepo].cdvHeight;
	...
	if (!expandedCommit.loading) {
		...
		if (codeReviewPossible) {
			...
			document.getElementById('cdvCodeReview')!.addEventListener('click', (e) => {
				...
				if (sourceElem.classList.contains(CLASS_ACTIVE)) {
					sendMessage({ command: 'endCodeReview', repo: view.getCurrentRepo(), id: expandedCommit.codeReview!.id });
					view.endCodeReview();
				} else {
					const commitOrder = getCommitOrder(view, expandedCommit.commitHash, expandedCommit.compareWithHash === null ? expandedCommit.commitHash : expandedCommit.compareWithHash);
		...
		if (externalDiffPossible) {
			document.getElementById('cdvExternalDiff')!.addEventListener('click', () => {
				const expandedCommit = view.getExpandedCommit();
				const gitConfig = view.getGitConfig();
				if (expandedCommit === null || gitConfig === null || (gitConfig.diffTool === null && gitConfig.guiDiffTool === null)) return;
				const commitOrder = getCommitOrder(view, expandedCommit.commitHash, expandedCommit.compareWithHash === null ? expandedCommit.commitHash : expandedCommit.compareWithHash);
				runAction({
