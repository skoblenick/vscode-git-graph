function buildBranchContextMenuActions(
  view: GitGraphView,
  target: DialogTarget & RefTarget
): ContextMenuActions {
  const refName = target.ref,
    visibility = view.getConfig().contextMenuActionsVisibility.branch;
  ...
  return [
    [
      ...
      },
      {
        ...
        onClick: () => {
          dialog.showRefInput(
            ...
            (newName) => {
              runAction(
                { command: 'renameBranch', repo: currentRepo, oldName: refName, newName: newName },
                'Renaming Branch'
              );
            },
            target
      ...
      },
      {
        ...
        onClick: () => {
          let remotesWithBranch = gitRemotes.filter((remote) =>
            gitBranches.includes('remotes/' + remote + '/' + refName)
          );
          let inputs: DialogInput[] = [
            {
              type: DialogInputType.Checkbox,
              name: 'Force Delete',
              value: config.dialogDefaults.deleteBranch.forceDelete
            }
          ];
        ...
        onClick: () => {
          ...
          dialog.showForm(
            'Are you sure you want to push the branch <b><i>' +
              ...
              (multipleRemotes
                ? ''
                : ' to the remote <b><i>' + escapeHtml(gitRemotes[0]) + '</i></b>') +
              '?',
            inputs,
            'Yes, push',
            (values) => {
              ...
              runAction(
                {
                  ...
                  willUpdateBranchConfig:
                    ...
                    remotes.length > 0 &&
                    (gitConfig === null ||
                      typeof gitConfig.branches[refName] === 'undefined' ||
                      gitConfig.branches[refName].remote !== remotes[remotes.length - 1])
                },
                'Pushing Branch'
              );
            },
    ...
    [
      {
        ...
        onClick: () => {
          runAction(
            { command: 'createArchive', repo: currentRepo, ref: refName },
            'Creating Archive'
          );
        }
      },
...
function buildCommitContextMenuActions(
  view: GitGraphView,
  target: DialogTarget & CommitTarget
): ContextMenuActions {
  const hash = target.hash,
    visibility = view.getConfig().contextMenuActionsVisibility.commit;
  ...
  return [
    [
      {
        ...
        onClick: () =>
          addTagAction(view, hash, '', config.dialogDefaults.addTag.type, '', null, target)
      },
      {
        title: 'Create Branch' + ELLIPSIS,
        visible: visibility.createBranch,
        onClick: () =>
          createBranchAction(view, hash, '', config.dialogDefaults.createBranch.checkout, target)
      }
    ],
    [
      {
        ...
        onClick: () => {
          const checkoutCommit = () =>
            runAction(
              { command: 'checkoutCommit', repo: currentRepo, commitHash: hash },
              'Checking out Commit'
            );
        ...
        onClick: () => {
          ...
          if (isMerge) {
            let options = commit.parents.map((hash, index) => ({
              name:
                abbrevCommit(hash) +
                (typeof commitLookup[hash] === 'number'
                  ? ': ' + commits[commitLookup[hash]].message
                  : ''),
        ...
        onClick: () => {
          if (commit.parents.length > 1) {
            ...
            dialog.showConfirmation(
              ...
              () => {
                runAction(
                  { command: 'revertCommit', repo: currentRepo, commitHash: hash, parentIndex: 0 },
                  'Reverting Commit'
                );
              },
              target
      ...
      },
      {
        ...
        onClick: () => {
          dialog.showConfirmation(
            ...
            () => {
              runAction(
                { command: 'dropCommit', repo: currentRepo, commitHash: hash },
                'Dropping Commit'
              );
            },
            target
    ...
    [
      ...
      },
      {
        ...
        onClick: () =>
          rebaseAction(view, hash, abbrevCommit(hash), GG.RebaseActionOn.Commit, target)
      },
      {
        ...
        onClick: () => {
          dialog.showSelect(
            'Are you sure you want to reset ' +
              (gitBranchHead !== null
                ? '<b><i>' + escapeHtml(gitBranchHead) + '</i></b> (the current branch)'
                : 'the current branch') +
              ' to commit <b><i>' +
              abbrevCommit(hash) +
              '</i></b>?',
            config.dialogDefaults.resetCommit.mode,
            [
...
function buildRemoteBranchContextMenuActions(
  view: GitGraphView,
  remote: string,
  target: DialogTarget & RefTarget
): ContextMenuActions {
  const refName = target.ref,
    visibility = view.getConfig().contextMenuActionsVisibility.remoteBranch;
    ...
    [
      ...
      },
      {
        ...
        visible:
          visibility.fetch &&
          remote !== '' &&
          gitBranches.includes(branchName) &&
          gitBranchHead !== branchName,
        onClick: () => {
          dialog.showForm(
      ...
      },
      {
        ...
        visible: visibility.merge,
        onClick: () =>
          mergeAction(view, refName, refName, GG.MergeActionOn.RemoteTrackingBranch, target)
      },
      {
            ...
            [
              {
                ...
                value: config.dialogDefaults.pullBranch.noFastForward
              },
              {
                type: DialogInputType.Checkbox,
                name: 'Squash Commits',
                value: config.dialogDefaults.pullBranch.squash,
    ...
    [
      buildViewIssueAction(view, refName, visibility.viewIssue, target),
      {
        ...
        visible:
          ...
          gitRepos[currentRepo].pullRequestConfig !== null &&
          branchName !== 'HEAD' &&
          (gitRepos[currentRepo].pullRequestConfig!.sourceRemote === remote ||
            gitRepos[currentRepo].pullRequestConfig!.destRemote === remote),
        onClick: () => {
          const prConfig = gitRepos[currentRepo].pullRequestConfig;
          if (prConfig === null) return;
...
function buildStashContextMenuActions(
  view: GitGraphView,
  target: DialogTarget & RefTarget
): ContextMenuActions {
  const hash = target.hash,
    selector = target.ref,
    visibility = view.getConfig().contextMenuActionsVisibility.stash;
  const currentRepo = view.getCurrentRepo();
  const config = view.getConfig();
  return [
    [
      ...
      },
      {
        ...
        onClick: () => {
          dialog.showConfirmation(
            ...
            () => {
              runAction(
                { command: 'dropStash', repo: currentRepo, selector: selector },
                'Dropping Stash'
              );
            },
            target
...
function buildTagContextMenuActions(
  view: GitGraphView,
  isAnnotated: boolean,
  target: DialogTarget & RefTarget
): ContextMenuActions {
  const hash = target.hash,
    tagName = target.ref,
    visibility = view.getConfig().contextMenuActionsVisibility.tag;
  const currentRepo = view.getCurrentRepo();
  const gitRemotes = view.getGitRemotes();
  return [
    [
      {
        ...
        onClick: () => {
          runAction(
            { command: 'tagDetails', repo: currentRepo, tagName: tagName, commitHash: hash },
      ...
      },
      {
        ...
        onClick: () => {
          let message =
            'Are you sure you want to delete the tag <b><i>' + escapeHtml(tagName) + '</i></b>?';
            ...
            dialog.showSelect(
              ...
              (remoteIndex) => {
                deleteTagAction(
                  view,
                  tagName,
                  remoteIndex !== '-1' ? gitRemotes[parseInt(remoteIndex)] : null
                );
              },
              target
            );
    ...
    [
      {
        ...
        onClick: () => {
          runAction(
            { command: 'createArchive', repo: currentRepo, ref: tagName },
            'Creating Archive'
          );
        }
      },
...
function buildUncommittedChangesContextMenuActions(
  view: GitGraphView,
  target: DialogTarget & CommitTarget
): ContextMenuActions {
          ...
          dialog.showForm(
            ...
            [
              { type: DialogInputType.Text, name: 'Message', default: '', placeholder: 'Optional' },
              {
                ...
                value: config.dialogDefaults.stashUncommittedChanges.includeUntracked,
    ...
    [
      {
        ...
        onClick: () => {
          dialog.showSelect(
            ...
            config.dialogDefaults.resetUncommitted.mode,
            [
              { name: 'Mixed - Keep working tree, but reset index', value: GG.GitResetMode.Mixed },
              { name: 'Hard - Discard all changes', value: GG.GitResetMode.Hard }
            ],
      ...
      },
      {
        ...
        onClick: () => {
          dialog.showCheckbox(
            ...
            (directories) => {
              runAction(
                { command: 'cleanUntrackedFiles', repo: currentRepo, directories: directories },
...
function buildViewIssueAction(
  ...
  if (
    visible &&
    (issueLinking = parseIssueLinkingConfig(gitRepos[currentRepo].issueLinkingConfig)) !== null
  ) {
...
function addTagAction(
  ...
  for (let i = 0; i < commits.length; i++) {
    if (
      commits[i].tags.length > 0 &&
      (mostRecentTagsIndex === -1 || commits[i].date > commits[mostRecentTagsIndex].date)
    ) {
  ...
  const mostRecentTags =
    mostRecentTagsIndex > -1
      ? commits[mostRecentTagsIndex].tags.map((tag) => '"' + tag.name + '"')
      : [];
  ...
  if (gitRemotes.length > 1) {
    ...
    const defaultValue =
      initialPushToRemote !== null || (isInitialLoad && config.dialogDefaults.addTag.pushToRemote);
    inputs.push({
      type: DialogInputType.Checkbox,
      name: 'Push to remote',
      value: defaultValue,
  ...
  dialog.showForm(
    ...
    (values) => {
      ...
      const pushToRemote =
        gitRemotes.length > 1 && <string>values[3] !== '-1'
    ...
    [
      {
        type: DialogInputType.Checkbox,
        name: 'Create a new commit even if fast-forward is possible',
        value: config.dialogDefaults.merge.noFastForward
      },
      {
        type: DialogInputType.Checkbox,
        name: 'Squash Commits',
        value: config.dialogDefaults.merge.squash,
        info:
      ...
      },
      {
        type: DialogInputType.Checkbox,
        name: 'No Commit',
        value: config.dialogDefaults.merge.noCommit,
    ...
    [
      {
        ...
        value: config.dialogDefaults.rebase.interactive
      },
      {
        type: DialogInputType.Checkbox,
        name: 'Ignore Date',
        value: config.dialogDefaults.rebase.ignoreDate,
