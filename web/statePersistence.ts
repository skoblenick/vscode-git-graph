...
function saveRepoState(view: GitGraphView) {
  sendMessage({
    ...
    state: view.getGitRepos()[view.getCurrentRepo()]
  });
}

function saveColumnWidths(view: GitGraphView, columnWidths: GG.ColumnWidth[]) {
  view.getGitRepos()[view.getCurrentRepo()].columnWidths = [
    columnWidths[0],
    columnWidths[2],
    columnWidths[3],
    columnWidths[4]
...
function saveExpandedCommitLoading(
  view: GitGraphView,
  index: number,
  commitHash: string,
...
function restoreFromPrevState(
  view: GitGraphView,
  prevState: WebViewState | null
): GG.LoadGitGraphViewTo {
  if (
    prevState &&
    !prevState.currentRepoLoading &&
    typeof view.getGitRepos()[prevState.currentRepo] !== 'undefined'
  ) {
    ...
    view.getShowRemoteBranchesElem().checked = getShowRemoteBranches(
      view.getGitRepos()[prevState.currentRepo].showRemoteBranchesV2
    );
  }
