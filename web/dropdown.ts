...
class Dropdown {
  ...
  public selectOption(value: string) {
    ...
    if (
      ...
      !this.optionsSelected[optionIndex]
    ) {
  ...
  public unselectOption(value: string) {
    ...
    if (
      this.multipleAllowed &&
      optionIndex > -1 &&
      (this.optionsSelected[0] || this.optionsSelected[optionIndex])
    ) {
  ...
  private render() {
    ...
    for (let i = 0; i < this.options.length; i++) {
      ...
      html +=
        ...
        escapedName +
        '">' +
        (this.multipleAllowed && this.optionsSelected[i]
    ...
    this.currentValueElem.style.width =
      Math.max(
        Math.ceil(menuElemRect.width) +
          ((this.showInfo || this.multipleAllowed) && menuElemRect.height < 272 ? 0 : 12),
        138
      ) + 'px';
