...
class Dialog {
  ...
  public showForm(
    ...
    const multiCheckbox =
      multiElement && inputs.every((input) => input.type === DialogInputType.Checkbox);
    ...
    const getFormValues = () =>
      inputs.map((input, index) => {
        if (input.type === DialogInputType.Radio) {
          ...
          const elems = <NodeListOf<HTMLInputElement>>(
            document.getElementsByName('dialogInput' + index)
          );
    ...
    if (
      inputs.length > 0 &&
      (inputs[0].type === DialogInputType.Text || inputs[0].type === DialogInputType.TextRef)
    ) {
  ...
  public refresh(commits: ReadonlyArray<GG.GitCommit>) {
    ...
    const commitIndex = commits.findIndex(
      (commit) => commit.hash === (<CommitTarget | RefTarget>this.target).hash
    );
    if (commitIndex > -1) {
...
class CustomSelect {
    ...
    this.clickHandler = (e: MouseEvent) => {
      ...
      if (
        ...
        (this.optionsElem === null ||
          targetElem.closest('.customSelectOptions') !== this.optionsElem)
      ) {
        this.render(false);
        return;
      }
      ...
      if (targetElem.className === 'customSelectCurrent') {
        ...
        if (optionElem !== null) {
          ...
          this.setItemSelectedState(
            selectedOptionIndex,
            data.multiple ? !this.selected[selectedOptionIndex] : true
          );
    ...
    currentElem.addEventListener('keydown', (e) => {
      if (this.open && e.key === 'Tab') {
        ...
        if (e.key === 'ArrowUp' || e.key === 'ArrowLeft') {
          this.setItemSelectedState(
            this.lastSelected > 0 ? this.lastSelected - 1 : this.data.options.length - 1,
            true
          );
          ...
          this.setItemSelectedState(
            this.lastSelected < this.data.options.length - 1 ? this.lastSelected + 1 : 0,
            true
          );
          this.scrollOptionIntoView(this.lastSelected);
          handledEvent(e);
   ...
   */
  public getValue() {
    const values = this.data.options
  ...
  private render(open: boolean) {
    ...
    if (this.open !== open) {
      ...
      if (open) {
        ...
        this.optionsElem.style.top =
          currentElemRect.top - dialogElemRect.top + currentElemRect.height - 2 + 'px';
        ...
        this.optionsElem.style.maxHeight =
          Math.max(
            document.body.clientHeight - currentElemRect.top - currentElemRect.height - 2,
            50
          ) + 'px';
        this.optionsElem.className =
          'customSelectOptions' + (this.data.multiple ? ' multiple' : '');
        const icon = this.data.multiple
          ? '<div class="selectedIcon">' + SVG_ICONS.check + '</div>'
          : '';
        this.optionsElem.innerHTML = this.data.options
          .map(
            (option, index) =>
              '<div class="customSelectOption" data-index="' +
              index +
              '">' +
              icon +
              escapeHtml(option.name) +
