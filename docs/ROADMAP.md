...
| Migrate Jest → Vitest                    | ✅     | Migrated to Vitest 4.x. Removed jest/ts-jest/jest-cli/@types/jest/jest-environment-jsdom           |
...
| 2026-03-20 | **Migrate Jest → Vitest**: Migrated from Jest 29 to Vitest 4.x. Replaced jest.fn/spyOn/mock → vi.fn/spyOn/mock. Used vi.mock with importOriginal for Node builtins (child_process, path, os, fs, iconv-lite). Global vscode alias in vitest.config.ts replaces per-file jest.mock('vscode', ..., {virtual: true}). Converted arrow fn mocks to regular functions for constructor support in Vitest 4. Removed jest, ts-jest, jest-cli, @types/jest, jest-environment-jsdom, jest.config.js. All 1638 tests pass across 29 suites. |
