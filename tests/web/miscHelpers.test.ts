...
describe('miscHelpers', () => {
  ...
  describe('haveFilesChanged', () => {
    it('should return true when old is null and new is not', () => {
      const result = vm.runInContext(
        'haveFilesChanged(null, [{ additions: 1, deletions: 0, newFilePath: "a.ts", oldFilePath: "a.ts", type: "A" }])',
        ctx
      );
    ...
    it('should return true when old is not null and new is null', () => {
      const result = vm.runInContext(
        'haveFilesChanged([{ additions: 1, deletions: 0, newFilePath: "a.ts", oldFilePath: "a.ts", type: "A" }], null)',
        ctx
      );
      expect(result).toBe(true);
    });
  ...
  describe('getRepoDropdownOptions', () => {
    it('should return options for a single repo', () => {
      ...
      expect(result).toEqual([{ name: 'project', value: '/home/user/project', hint: '' }]);
    });
