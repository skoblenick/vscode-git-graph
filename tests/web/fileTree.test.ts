...
describe('fileTree', () => {
	...
	beforeEach(() => {
		...
		sandbox.GG.GitFileStatus = { Added: 'A', Modified: 'M', Deleted: 'D', Renamed: 'R', Untracked: 'U' };
	...
	describe('setFileTreeReviewed', () => {
		...
		it('should mark all files and folders as not reviewed', () => {
			...
			expect(vm.runInContext('srFolder2.contents["sub"].contents["b.ts"].reviewed', ctx)).toBe(false);
		});
	...
	describe('alterFileTreeFileReviewed', () => {
		...
		it('should recalculate parent folder reviewed status to false when child is unreviewed', () => {
			...
			`, ctx);
			expect(vm.runInContext('afFolder2.contents["src"].contents["file.ts"].reviewed', ctx)).toBe(false);
		...
		it('should recalculate parent folder reviewed status to true when all children are reviewed', () => {
			...
			`, ctx);
			expect(vm.runInContext('afFolder3.contents["src"].contents["file.ts"].reviewed', ctx)).toBe(true);
