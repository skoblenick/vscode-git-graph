...
function createMockView(ctx: vm.Context, sandbox: Record<string, any>, overrides: Record<string, any> = {}) {
	const defaults: Record<string, any> = {
		...
		gitRemotes: ['origin'],
		gitTags: ['v1.0'],
		commits: [{ hash: 'abc123def456789000000000000000000000000a', message: 'test', parents: ['def456'], tags: [], stash: null, date: 1000 }],
		commitLookup: { 'abc123def456789000000000000000000000000a': 0 },
		config: { ...sandbox.initialState.config, contextMenuActionsVisibility: allVisibility(), dialogDefaults: allDialogDefaults(), fetchAndPrune: false, fetchAndPruneTags: false },
...
function createTarget(ctx: vm.Context, overrides: Record<string, any> = {}) {
	...
	ctx.mockTarget = { type: 'ref', elem, hash: 'abc123def456789000000000000000000000000a', ref: 'feature', ...overrides };
...
describe('contextMenuActions', () => {
	...
	describe('buildBranchContextMenuActions', () => {
		...
		it('should send correct message when Copy Branch Name is clicked', () => {
			...
			copyAction.onClick();
			expect(vsCodeApi.postMessage).toHaveBeenCalledWith({ command: 'copyToClipboard', type: 'Branch Name', data: 'feature' });
		});
	...
	describe('buildCommitContextMenuActions', () => {
		...
		it('should send correct message when Copy Commit Hash is clicked', () => {
			...
			expect(copyAction.visible).toBe(true);
			copyAction.onClick();
			expect(vsCodeApi.postMessage).toHaveBeenCalledWith({ command: 'copyToClipboard', type: 'Commit Hash', data: 'abc123def456789000000000000000000000000a' });
		});
	});
	...
	describe('buildRemoteBranchContextMenuActions', () => {
		...
		it('should extract remote branch name correctly for Copy action', () => {
			...
			expect(copyAction.visible).toBe(true);
			copyAction.onClick();
			expect(vsCodeApi.postMessage).toHaveBeenCalledWith({ command: 'copyToClipboard', type: 'Branch Name', data: 'origin/feature' });
		});
	});
	...
	describe('buildStashContextMenuActions', () => {
		it('should return correct structure with visible actions', () => {
			createMockView(ctx, sandbox);
			createTarget(ctx, { ref: 'refs/stash@{0}', hash: 'abc123def456789000000000000000000000000a' });
		...
		it('should send correct message when Copy Stash Name is clicked', () => {
			...
			expect(copyAction.visible).toBe(true);
			copyAction.onClick();
			expect(vsCodeApi.postMessage).toHaveBeenCalledWith({ command: 'copyToClipboard', type: 'Stash Name', data: 'refs/stash@{0}' });
		});
	});
	...
	describe('buildTagContextMenuActions', () => {
		...
		it('should not show View Details for lightweight tag', () => {
			createMockView(ctx, sandbox);
			createTarget(ctx, { ref: 'v1.0', hash: 'abc123def456789000000000000000000000000a' });
			const result = vm.runInContext('buildTagContextMenuActions(mockView, false, mockTarget)', ctx);
			const viewDetails = result[0].find((a: any) => a.title === 'View Details');
	...
	describe('buildUncommittedChangesContextMenuActions', () => {
		it('should return correct structure', () => {
			createMockView(ctx, sandbox);
			createTarget(ctx, { hash: 'abc123def456789000000000000000000000000a' });
			const result = vm.runInContext('buildUncommittedChangesContextMenuActions(mockView, mockTarget)', ctx);
			expect(Array.isArray(result)).toBe(true);
	...
	describe('fetchFromRemotesAction', () => {
		it('should call runAction with correct parameters', () => {
			...
			expect(vsCodeApi.postMessage).toHaveBeenCalledWith(
				{ command: 'fetch', repo: '/repo', name: null, prune: false, pruneTags: false }
			);
		});
	});
