...
describe('RepoManager', () => {
    ...
    it('Should remove repositories contained within a removed workspace folder', async () => {
      ...
      const repoManager = await constructRepoManagerAndWaitUntilStarted(
        ...
        [
          ...
          '/path/to/workspace-folder2/submodule'
        ]
      );
  ...
  describe('startupTasks', () => {
    it('Should run startup tasks', async () => {
      ...
      const repoManager = await constructRepoManagerAndWaitUntilStarted(
        [
          '/path/to/workspace-folder1',
          '/path/to/another/workspace-folder',
          '/path/to/workspace-folder3'
        ],
        [
  ...
  describe('registerRepo', () => {
    ...
    it('Should return an error message when the path being registered is contained within a known Git repository', async () => {
      ...
      // Run
      const result = await repoManager.registerRepo(
        '/path/to/workspace-folder1/subdirectory',
        false
      );
  ...
  describe('getRepoContainingFile', () => {
    ...
    it('Should return the path of the closest repository that contains the file', async () => {
      ...
      // Run
      const result = repoManager.getRepoContainingFile(
        '/path/to/workspace-folder1/repo1/subrepo/file.txt'
      );
  ...
  describe('searchWorkspaceForRepos', () => {
    ...
    it('Should add repositories that are detected in subfolders of the workspace', async () => {
      ...
      expect(spyOnStat).toHaveBeenCalledWith(
        '/path/to/workspace-folder1/file.txt',
        expect.anything()
      );
    ...
    it('Should halt recursion when fs.readdir returns an error', async () => {
      ...
      await waitForExpect(() =>
        expect(spyOnLog).toHaveBeenCalledWith('Completed searching workspace for new repos')
      );
  ...
  describe('onWatcherChange', () => {
    ...
    it("Shouldn't remove a repository when a directory is removed, but it doesn't contain any repositories", async () => {
      ...
      await waitForExpect(() => {
        ...
        expect(repoManager['onWatcherChangeQueue']['processing']).toBe(false);
        expect(spyOnStat).toHaveBeenCalledWith(
          '/path/to/workspace-folder1/dir/repo',
          expect.anything()
        );
        expect(repoManager.getRepos()).toStrictEqual({
          '/path/to/workspace-folder1/repo': mockRepoState({ workspaceFolderIndex: 0 })
  ...
  describe('onWatcherDelete', () => {
    ...
    it('Should delete repositories within a deleted repository', async () => {
      ...
      expect(spyOnLog).toHaveBeenCalledWith(
        'Removed repo: /path/to/workspace-folder1/repo1/submodule'
      );
      expect(repoManager.getRepos()).toStrictEqual({
        '/path/to/workspace-folder1/repo2': mockRepoState({ workspaceFolderIndex: 0 })
      });
    ...
    it('Should not proceed to remove repositories if the deleted URI is within .git', async () => {
      ...
      const repoManager = await constructRepoManagerAndWaitUntilStarted(
        ...
        [
          '/path/to/workspace-folder1/dir/repo1',
          '/path/to/workspace-folder1/dir/repo1/.git/folder/repo'
      ...
      // Assert
      expect(spyOnLog).not.toHaveBeenCalledWith(
        '/path/to/workspace-folder1/dir/repo1/.git/folder/repo'
      );
      expect(repoManager.getRepos()).toStrictEqual({
        '/path/to/workspace-folder1/dir/repo1': mockRepoState({ workspaceFolderIndex: 0 }),
        '/path/to/workspace-folder1/dir/repo1/.git/folder/repo': mockRepoState({
          workspaceFolderIndex: 0
        })
      });
      expect(onDidChangeReposEvents).toStrictEqual([]);
  ...
  describe('checkRepoForNewConfig', () => {
    describe('apply config', () => {
      ...
      describe('fileViewType', () => {
        it(
          ...
          testApplyField(
            ...
            FileViewType.Tree,
            'fileViewType',
            ExternalRepoConfig.FileViewType.Tree
          )
        );
        it(
          ...
          testApplyField(
            ...
            FileViewType.List,
            'fileViewType',
            ExternalRepoConfig.FileViewType.List
          )
        );
      });
      ...
      describe('hideRemotes', () => {
        it(
          'Should import hideRemotes correctly',
          testApplyField('hideRemotes', ['origin'], 'hideRemotes', ['origin'])
        );
      });
      ...
      describe('issueLinkingConfig', () => {
        it(
          ...
          testApplyField('issueLinkingConfig', { issue: 'x', url: 'y' }, 'issueLinkingConfig', {
            issue: 'x',
            url: 'y'
          })
        );
    ...
    describe('validation', () => {
      ...
      it(
        'Should display a validation error when "name" is invalid',
        testValidationOfField('name', 5)
      );
      it(
      ...
      it(
        'Should display a validation error when "showTags" is invalid',
        testValidationOfField('showTags', 'invalid')
      );
    });
  ...
  describe('onConfigWatcherCreateOrChange', () => {
    it('Should import the repository configuration when the file is created', async () => {
      ...
      // Run
      emitOnDidCreate(
        vscode.Uri.file('/path/to/workspace-folder1/repo1/.vscode/vscode-git-graph.json')
      );
    ...
    it('Shouldn\'t import the repository configuration when user responds "No"', async () => {
      ...
      // Assert
      await waitForExpect(() =>
        expect(spyOnIsKnownRepo).toHaveBeenCalledWith('/path/to/workspace-folder1/repo1')
      );
    ...
    it("Shouldn't import the repository configuration when it is not for a known repository", async () => {
      ...
      // Run
      emitOnDidChange(
        vscode.Uri.file('/path/to/workspace-folder1/repo2/.vscode/vscode-git-graph.json')
      );
  ...
  describe('exportRepoConfig', () => {
    it('Should export the repository configuration', async () => {
      ...
      // Assert
      expect(result).toBe(null);
      expect(utils.getPathFromStr(spyOnMkdir.mock.calls[0][0])).toBe(
        '/path/to/workspace-folder1/repo1/.vscode'
      );
    ...
    describe('hideRemotes', () => {
      it(
        'Should export hideRemotes correctly',
        testExportField('hideRemotes', ['origin'], 'hideRemotes', ['origin'])
      );
    });
    ...
    describe('issueLinkingConfig', () => {
      it(
        ...
        testExportField('issueLinkingConfig', { issue: 'x', url: 'y' }, 'issueLinkingConfig', {
          issue: 'x',
          url: 'y'
        })
      );
...
function mockRepository(generateRootPath?: (path: string) => string) {
  spyOnRepoRoot.mockImplementationOnce((path) =>
    Promise.resolve(generateRootPath ? generateRootPath(path) : path)
  );
}
...
function mockFsMkdirOnce(err: NodeJS.ErrnoException | null) {
  spyOnMkdir.mockImplementationOnce(
    (_: string, callback: (err: NodeJS.ErrnoException | null) => void) => {
      callback(err);
    }
  );
}
